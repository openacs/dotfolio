...
select define_function_args ('dotfolio_user_profile_rel__new','rel_id,user_id,id,rel_type;dotfolio_user_profile_rel,group_id,creation_user,creation_ip');

create function dotfolio_user_profile_rel__new(integer,integer,varchar,varchar,integer,integer,varchar)
returns integer as '
DECLARE
...
BEGIN
        ...
        insert
        into dotfolio_user_profile_rels
        (rel_id, id)
        values
        (v_rel_id, p_id);
...
select define_function_args ('dotfolio_user_profile_rel__delete','rel_id');

create function dotfolio_user_profile_rel__delete(integer)
returns integer as '
DECLARE
        p_rel_id                alias for $1;
BEGIN

...
select define_function_args ('dotfolio_admin_profile_rel__new','rel_id,user_id,id,rel_type;dotfolio_admin_profile_rel,group_id,creation_user,creation_ip');

create function dotfolio_admin_profile_rel__new(integer,integer,varchar,varchar,integer,integer,varchar)
returns integer as '
DECLARE
...
BEGIN
        ...
        insert
        into dotfolio_admin_profile_rels
        (rel_id)
        values
        (v_rel_id);
...
select define_function_args ('dotfolio_admin_profile_rel__delete','rel_id');

create function dotfolio_admin_profile_rel__delete(integer)
returns integer as '
DECLARE
        p_rel_id                alias for $1;
BEGIN
        delete
        from dotfolio_admin_profile_rels

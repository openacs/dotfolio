...
select define_function_args ('dotfolio_guest_profile_rel__new','rel_id,user_id,id,rel_type;dotfolio_guest_profile_rel,group_id,creation_user,creation_ip');

create function dotfolio_guest_profile_rel__new(integer,integer,varchar,varchar,integer,integer,varchar)
returns integer as '
DECLARE
...
BEGIN
        if p_group_id is null then
            ...
            where profile_provider = (select min(impl_id)
                                      from acs_sc_impls
                                      where impl_name = ''dotfolio_guest_profile_provider'');
        else
        ...
        insert
        into dotfolio_guest_profile_rels
        (rel_id)
        values
        (v_rel_id);
...
select define_function_args ('dotfolio_guest_profile_rel__delete','rel_id');

create function dotfolio_guest_profile_rel__delete(integer)
returns integer as '
DECLARE
        p_rel_id                alias for $1;
BEGIN
        delete
        from dotfolio_guest_profile_rels
